              ...
              params: [
                {
                  ...
                  rpcUrls: ['https://goerli.infura.io/v3/6c4641ed2a424efba3f4c6b04b3845d9'],
                },
              ],
            });
          } else if(chain === 'polygon') {
...
async function loadInfo() {
  ...
  if (publicMintActive) {
    ...
    try {
      ...
      const whitelisted = await contract.methods.isWhitelisted(window.address.toLowerCase(), merkleJson).call();

      console.log("whitelisted: ", whitelisted, "addy: ", window.address, "json: ", merkleJson, "Contract: ", contract);
      if(!whitelisted) {
  ...
  let priceType = '';
  if(chain === 'rinkeby' || chain === 'ethereum' || chain === 'goerli') {
    priceType = 'ETH';
  } else if (chain === 'polygon') {
    priceType = 'MATIC';
  }
